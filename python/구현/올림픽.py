...
nList = sorted([list(map(int, sys.stdin.readline().split())) for i in range(0, n)] , key=lambda x :(x[1], x[2], x[3]), reverse=True)

print(nList)

for i in nList:
    ansdict[i[0]] = ansdict.get(i[0], i[1:])
...
t = list(set(tuple(tuple(i) for i in vList)))
